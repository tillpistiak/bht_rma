...
jQuery(document).ready(function ($) {
  ...
  $.fn.pressEnter = function (callback) {
    return this.each(function () {
      //Loop über alle Elemente der jQuery-Selektion
      $(this).on("enterPress", callback); //Callback-Funktion für unser Custom Event registrieren
      $(this).keypress(function (e) {
        //keypress-Handler hinzufügen
        if (e.keyCode == 13) {
          //ist gedrückte Taste ENTER ?
  ...
  $('ul.todo-list li input[type="text"]').pressEnter(handleEnter);
  ...
  function createNewItem() {
    onArrowClick(null); // Menue schliessen, falls geoeffnet
    let todoList = $(".todo-list");
    let newItem = $(listElementTemplate);
    let newCheckbox = newItem.find('input[type="checkbox"]');
    todoList.append(newItem);
    setCheckboxAttributes(newCheckbox, uniqueItemID++);
    styleCheckbox(newCheckbox);
    setEditModeItem(newItem);
    fadeInItem(newItem);
    addHandlersForNewItem(newItem);
  }

  function addHandlersForNewItem(newItem) {
    ...
    newItem.find('input[type="text"]').pressEnter(handleEnter);
  }

  function handleEnter(event) {
    if (editItem) {
      if (event.target == editItem.find('input[type="text"]')[0]) {
        unEditModeItem();
      }
    }
  }
